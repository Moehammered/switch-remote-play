...
void initTest()
{
	...
	//now let's press up for about 10 seconds
	DS4_REPORT state;
	ZeroMemory(&state, sizeof(DS4_REPORT));
	DS4_REPORT_INIT(&state);
	state.wButtons = DS4_BUTTON_CROSS;
	int counter = 10;

	

	Sleep(3000); //let the controller register
	int choice = -1;
	int switcher = 0;
	do
	{
		do
		{
			auto direction = ++switcher % 2 == 0 ? DS4_BUTTON_DPAD_NORTH : DS4_BUTTON_DPAD_SOUTH;
			DS4_SET_DPAD(&state, direction);
			vigem_target_ds4_update(client, pad, state);
			cout << "Pressed 'up' or 'down'" << endl;
			Sleep(50);
			cout << "reset dpad state" << endl;
			DS4_SET_DPAD(&state, DS4_BUTTON_DPAD_NONE);
			vigem_target_ds4_update(client, pad, state);
			Sleep(1000);
		} while (--counter >= 0);
		cout << "Restart input? (0 = yes)" << endl;
		cin >> choice;
	} while (choice == 0);
