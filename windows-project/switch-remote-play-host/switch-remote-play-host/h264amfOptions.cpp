...
namespace h264amf
{
    ...
    H264AMFData CodecParamsFromStr(std::unordered_map<Parameters, std::string> const& map)
    {
        ...
        parse(Parameters::Usage, data.usage, H264AMF_USAGE::LOWLATENCY, amfUsageStrToEnum);
        parse(Parameters::Profile, data.profile, H264AMF_PROFILE::MAIN, amfProfileStrToEnum);
        parse(Parameters::Level, data.level, levelDefault, amfLevelStrToInt);
        parse(Parameters::Quality, data.quality, H264AMF_QUALITY::BALANCED, amfQualityStrToEnum);
        parse(Parameters::RateControl, data.rateControl, H264AMF_RATECONTROL::CQP, amfRateControlStrToEnum);
        parse(Parameters::FrameQuant_I, data.qp_i, qpFrameDefault, amfFrameQPStrToInt);
        parse(Parameters::FrameQuant_P, data.qp_p, qpFrameDefault, amfFrameQPStrToInt);
